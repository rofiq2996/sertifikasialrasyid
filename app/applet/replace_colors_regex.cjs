...
const regex = /(?:text|bg|border|ring|fill|from|via|to)-amber-(50|100|200|300|400|500|600|700|800|900|950)/g;
replaceInDir('src', regex, (match, shade) => {
    // Preserve semantic lightness roughly by substituting with the target color but with opacities for lighter shades
    let prefix = match.split('-amber-')[0]; // text, bg, border...
...
console.log('Replaced amber colors inside src/');
